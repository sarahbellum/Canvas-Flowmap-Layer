...
The Canvas-Flowmap-Layer extends the Esri JSAPI to map the flow of objects from an origin point to a destination point by using a Bezier curve. Esri graphics are translated to pixel space so that rendering for the points and curves are mapped to the canvas.

## API

Extends JSAPI 3.x [esri/layers/GraphicsLayer](https://developers.arcgis.com/javascript/3/jsapi/graphicslayer-amd.html). All properties, methods, and events provided by the `GraphicsLayer` should be available in this `CanvasFlowmapLayer`.
...
| `animatePathProperties` | _Optional_. `Object`. This defines the properties of the animated Bezier curve that is drawn on the canvas directly on top of the non-animated Bezier curve. [See Line Animation below](#Line Animation-example)  |
